...
class FormControllerBoxTests {
    ...
    @Test
    fun `GIVEN the form controller has no boxes WHEN a box is added THEN the field snapshot can be accessed`() = runTest {
    ...
    @Test
    fun `GIVEN A box flow for a field not present in the form controller has been obtained WHEN a box is added THEN the field is emitted to the flow`() = runTest{
        val property = randomProperty()
        val formController = testFormController()
    ...
    @Test
    fun `GIVEN the form controller has no boxes, WHEN a box is added THEN calling field provides the form box`() = runTest{
        val property = randomProperty()
        val formController = testFormController()
    ...
    @Test
    fun `GIVEN the form controller has a box WHEN the clear function is called THEN the box is removed`() = runTest {
        ...
        val boxFlow = formController.state.boxFlows.toList().first().second
        val observerJob = formController.state.observerJobs.toList().first().second
    ...
    @Test
    fun `GIVEN the form controller has 2 boxes WHEN the clear function is called with one box THEN the box is removed and the other box remains`() = runTest {
        ...
        assertEquals(property2.name, formController.state.boxes.toList().first().first)

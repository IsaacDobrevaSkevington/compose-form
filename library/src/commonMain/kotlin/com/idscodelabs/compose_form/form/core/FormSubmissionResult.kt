...
interface FormSubmissionResult<Model> {
    ...
    class Error<Model>(private val error: Throwable): FormSubmissionResult<Model>{
        override fun onError(action: (Throwable) -> Unit): FormSubmissionResult<Model> {
            action(error)
            return super.onError(action)
        }
    }
    class Success<Model>(private val model: Model): FormSubmissionResult<Model>{
        override fun onSuccess(action: (Model) -> Unit): FormSubmissionResult<Model> {
            action(model)
            return super.onSuccess(action)
        }
    }
    class Failure<Model>(private val boxes: List<FormBox<Model, *>>): FormSubmissionResult<Model>{
